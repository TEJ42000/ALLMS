# tests/test_ai_tutor.py - Tests for AI Tutor endpoints
...
class TestChatEndpoint:
    ...
    def test_chat_with_conversation_history(self, client, sample_chat_request_with_history, mock_tutor_response):
        """Test chat request with conversation history."""
        with patch('app.services.anthropic_client.client') as mock_client:
            mock_client.messages.create = AsyncMock(return_value=mock_tutor_response)
            
            response = client.post("/api/tutor/chat", json=sample_chat_request_with_history)
