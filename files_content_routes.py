# app/routes/files_content.py - API Routes using Anthropic Files API
...
class FilesQuizRequest(BaseModel):
    ...
    topic: str = Field(..., description="Constitutional, Administrative, Criminal, Private, or International Law")
...
async def generate_quiz_from_files(request: FilesQuizRequest):
    ...
    **Example:**
    ...
    try:
        ...
        if request.week:
            lecture_key = f"lecture_week_{request.week}"
...
async def explain_article(request: ArticleExplainRequest):
    ...
    **Example:**
    ...
    try:
        ...
        return {
            "article": f"Art. {request.article} {request.code}",
            "explanation": explanation
        }
        
    except Exception as e:
        logger.error(f"Error explaining article: {e}")
        raise HTTPException(500, detail=str(e))
...
async def get_topic_files(topic: str):
    ...
    try:
        ...
        logger.error(f"Error getting topic files: {e}")
        raise HTTPException(500, detail=str(e))
...
async def check_files_api_status():
    ...
    try:
        ...
        return {
            "status": "ok" if (files_loaded and api_accessible) else "error",
            "files_loaded": files_loaded,
            "local_file_count": len(service.file_ids),
            "api_accessible": api_accessible,
            "api_file_count": api_file_count,
            "message": "Files API ready!" if (files_loaded and api_accessible) else "Run upload_files_script.py first"
        }
